...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, MatIconModule, MatProgressBarModule, MatTabsModule, DagEditorComponent, WorldPreviewComponent, WorldEditorComponent],
  template: `
...
export class CaseEditorComponent implements OnInit {
  ...
  confirmDeleteNode(caseVersionId: number, node: NodeEditorState) {
    if (!confirm(`¿Eliminar el nodo "${node.title}"? Esta acción eliminará también sus decisiones, mapa y objetos.`)) return;
    this.busy.set(true);
  ...
  confirmDeleteMap(caseVersionId: number, map: MapEditorState) {
    if (!confirm(`¿Eliminar el mapa "${map.title}"? Esta acción eliminará también sus objetos y colisiones.`)) return;
    this.busy.set(true);
  ...
  confirmDeleteObject(caseVersionId: number, obj: MapObjectEditorState) {
    if (!confirm(`¿Eliminar el objeto "${obj.label}"?`)) return;
    this.busy.set(true);
  ...
  confirmDeleteTool(caseVersionId: number, tool: ClinicalToolEditorState) {
    if (!confirm(`¿Eliminar la herramienta "${tool.label}"?`)) return;
    this.busy.set(true);
